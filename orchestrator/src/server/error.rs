    ...
    fn into_response(self) -> Response {
        match self {
            JobRouteError::InvalidId(id) => {
                (StatusCode::BAD_REQUEST, Json(ApiResponse::<()>::error(format!("Invalid job ID: {}", id))))
                    .into_response()
            }
            JobRouteError::NotFound(id) => {
                (StatusCode::NOT_FOUND, Json(ApiResponse::<()>::error(format!("Job not found: {}", id))))
                    .into_response()
            }
            ...
            JobRouteError::InvalidJobState(msg) => {
                (StatusCode::CONFLICT, Json(ApiResponse::<()>::error(format!("Invalid job state: {}", msg))))
                    .into_response()
            }
            JobRouteError::DatabaseError => (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ApiResponse::<()>::error("Database error occurred".to_string())),
            )
                .into_response(),
